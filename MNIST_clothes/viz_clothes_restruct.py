        ...
        for epoch in range(5):
            ...
            try:
                kernel = tf.get_collection(
                    tf.GraphKeys.VARIABLES, 'layer1/kernel')[0]
                kernel = tf.get_collection(
                    tf.GraphKeys.VARIABLES, 'layer1/kernel')[0]
                print(kernel)
            except:
            ...
            try:
                ...
                print("getting variables with scope didn't work")

            try:
                print(sess.run(layer1_W))
            except:
                print("can't do sess run")
            """kernel = tf.get_collection(
                tf.GraphKeys.VARIABLES, 'layer1/kernel')[0]
            np.save("conv1_weights", kernel)"""
